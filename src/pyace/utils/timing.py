...
def run_timing_test(
    ...
    for atoms in atoms_list:
        ae = aseatoms_to_atomicenvironment(
            atoms, elements_mapper_dict=bbasis.elements_to_index_map, cutoff=rcut
        )
        ae_list.append(ae)
    ...
    for i, ae in enumerate(ae_list):
        ...
        if verbose:
            print(
                "\t{}/{}: {:.0f} mcs/atom".format(
                    i + 1, n_struct, time_res / n_iter / ae.n_atoms_real * 1e6
                )
                + " " * 40,
                end="\r",
            )
    ...
    if verbose:
        ...
        for proto, row in gdf.iterrows():
            ...
            print(
                "{:3.0f} (+/-{:2.0f})".format(
                    row[("time_per_atom", "mean")], row[("time_per_atom", "std")]
                )
            )
        print("=" * 80)

    tpa_mean, tpa_std = (
        gdf[("time_per_atom", "mean")].mean(),
        gdf[("time_per_atom", "mean")].std(),
    )
