...
class ParallelDataExecutor:
    ...
    def __init__(
        ...
        if self.parallel_mode != ParallelDataExecutor.MODE_PROCESS:
            warnings.warn(
                "ParallelDataExecutor: n_workers ({}) would be ignored for parallel_mode={}".format(
    ...
    def _split_data_indices_batches(self):
        self._data_indices_batches = split_by_batches(
            self._data_index, self._actual_batch_size
        )
    ...
    def start_executor(self):
        ...
        if self.parallel_mode == ParallelDataExecutor.MODE_MPI:
            ...
            self._executor = MPIPoolExecutor(
                globals={LOCAL_DATAFRAME_VARIALBE_NAME: self._distributed_df}
            )
            ...
            self._executor = ProcessPoolExecutor(
                ...
                initializer=local_dataframe_initializer,
            ...
            raise ValueError(
                "Unrecognized parallel_mode='{}'".format(self.parallel_mode)
            )
        self._actual_batch_size = max(
            1, min(self.batch_size, len(self._data_index) // self._n_workers)
        )
        log.info(
            "Start executor in '{}' mode with {} workers, actual batch size is {}".format(
                self.parallel_mode, self._n_workers, self._actual_batch_size
            )
        )
    ...
    def map(self, pure_row_func=None, wrapped_pure_func=None):
        ...
        if wrapped_pure_func is None:
            wrapped_pure_func = partial(
                batch_function_wrapper, pure_row_func=pure_row_func
            )
        for ind_range, val in zip(
            self._data_indices_batches,
