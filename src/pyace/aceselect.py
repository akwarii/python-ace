...
from pyace.activelearning import (
    ...
    read_extrapolation_data,
    compute_number_of_functions,
    count_number_total_atoms_per_species_type_aseatom,
    compute_active_set,
)
...
def select_structures_maxvol(
    ...
    total_number_of_atoms_per_species_type = (
        count_number_total_atoms_per_species_type_aseatom(
            ase_atoms_list, elements_to_index_map
        )
    )
    ...
    if batch_size is None:
        ...
        b_proj, structure_ind_dict = compute_B_projections(
            bconf, ase_atoms_list, return_structure_ind_dict=True, verbose=verbose
        )

        if verbose:
            log.info("Selecting structures")
        res_no_batch = compute_active_set(
            b_proj,
            structure_ind_dict,
            tol=gamma_tolerance,
            max_iters=maxvol_iters,
            extra_A0_projections_dict=extra_A0_projections_dict,
            verbose=verbose,
        )
        ...
        res_batch = compute_active_set_by_batches(
            bconf,
            ase_atoms_list,
            n_batches=n_batches,
...
def compute_required_memory(
    ...
    for st in total_number_of_atoms_per_species_type.keys():
        if verbose:
            log.info(
                "\tElement: {}, # atoms: {}, # B-func: {}, # projections: {}".format(
                    elements_name[st],
                    total_number_of_atoms_per_species_type[st],
                    nfuncs[st],
                    n_projections[st],
                )
            )
        number_of_projection_entries += (
            total_number_of_atoms_per_species_type[st] * n_projections[st]
        )
...
def load_multiple_datasets(dataset_filename, elements=None, verbose=False):
    ...
    for i, dsfn in enumerate(dataset_filename):
        ...
        if dsfn.endswith(".pckl.gzip") or dsfn.endswith(".pkl.gz"):
            ...
            if elements is None:
                raise ValueError(
                    '`elements` are missing. Provide it as in LAMMPS with --elements "A B C"'
                )
            species_to_element_dict = {i + 1: e for i, e in enumerate(elements.split())}
            # LAMMPS dat format
            structures = read_extrapolation_data(
                dsfn, species_to_element_dict=species_to_element_dict
            )
        ...
        if 2 * required_projections_memory + required_active_set_memory < mem_lim:
            ...
            nsplits = int(
                np.ceil(
                    2
                    * required_projections_memory
                    // (mem_lim - required_active_set_memory)
                )
            )
...
def save_selected_structures(df_selected, selected_structures_filename, verbose):
    if "POSCAR" in selected_structures_filename:
        if verbose:
            log.info(
                f"Saving selected structures to {selected_structures_filename.replace('POSCAR', '*.POSCAR')}"
            )
        os.makedirs(os.path.dirname(selected_structures_filename), exist_ok=True)
        for i, at in enumerate(df_selected["ase_atoms"]):
            fname = selected_structures_filename.replace(
                "POSCAR", "{}.POSCAR".format(i)
            )
            write(fname, at, format="vasp", sort=True)
    else:
        if verbose:
            log.info(
                f"Saving selected structures to {selected_structures_filename} as pickled dataframe"
            )
