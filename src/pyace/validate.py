...
def plot_analyse_error_distributions(
    df_pred, fig_prefix="train_", fig_path=".", imagetype="png"
):
    ...
    with warnings.catch_warnings():
        ...
        df_pred["energy_corrected_per_atom"] = df_pred[
            "energy_corrected_per_atom"
        ].astype(float)
        df_pred["energy_corrected"] = df_pred["energy_corrected"].astype(float)

        # energies
        df_pred["energy_pred_per_atom"] = (
            df_pred["energy_pred"] / df_pred["NUMBER_OF_ATOMS"]
        )
        ...
        ## 2. E-error dist plot
        ########################################
        fig, ((ax0, ax_inv), (ax1, ax2)) = plt.subplots(
            2,
            2,
            dpi=150,
            figsize=(7, 4),
            gridspec_kw={"width_ratios": [7, 1], "height_ratios": [1, 3]},
        )
        ...
        dExx = hist_res[1]
        ax2.plot(
            stats.norm.pdf(dExx, loc=dE_mean * 1e3, scale=dE_std * 1e3),
            dExx,
            color="orange",
            ls="--",
        )
        ...
        for bin_ind in range(min(bins_inds), max(bins_inds) + 1):
            de_per_bin[bin_ind] = np.sqrt(
                np.mean(dforces_flatten[bins_inds == bin_ind] ** 2)
            )
        ...
        dFxx = hist_res[1]
        ax2.plot(
            stats.norm.pdf(dFxx, loc=df_mean * 1e3, scale=df_std * 1e3),
            dFxx,
            color="orange",
            ls="--",
        ...
        ax1.scatter(
            forces_norm, dforces_norm * 1e3, s=3, label="sample", color="lightsteelblue"
        )
        ...
        for bin_ind in range(min(bins_inds), max(bins_inds) + 1):
            de_per_bin[bin_ind] = np.sqrt(
                np.mean(dforces_norm[bins_inds == bin_ind] ** 2)
            )
        de_per_bin = np.nan_to_num(de_per_bin, copy=False, nan=0)
        ax1.plot(bins, de_per_bin * 1e3, color="red", lw=2, label="RMSE")
