...
class MetricsAggregator:
    ...
    columns = [
        ("ladder_step", 11),
        ("cycle_step", 11),
        ("iter_num", 8),  # reduce column width
        "loss",
        "e_loss_contrib",
        "f_loss_contrib",
        "reg_loss",
    ...
    def __init__(
        ...
        if self.ladder_metrics_filename is not None:
            ...
            self.ladder_metrics_columns = (
                self.ladder_metrics_columns[:1] + self.ladder_metrics_columns[3:]
            )
            self.write_metric_table_title_to_file(
                self.ladder_metrics_filename, columns=self.ladder_metrics_columns
            )
        ...
        if self.cycle_metrics_filename is not None:
            ...
            self.cycle_metrics_columns = (
                self.cycle_metrics_columns[:2] + self.cycle_metrics_columns[3:]
            )
            self.write_metric_table_title_to_file(
                self.cycle_metrics_filename, columns=self.cycle_metrics_columns
            )
        ...
        if self.test_ladder_metrics_filename is not None:
            ...
            self.write_metric_table_title_to_file(
                self.test_ladder_metrics_filename, columns=self.ladder_metrics_columns
            )
        ...
        if self.test_cycle_metrics_filename is not None:
            ...
            self.write_metric_table_title_to_file(
                self.test_cycle_metrics_filename, columns=self.cycle_metrics_columns
            )
    ...
    def ladder_step_callback(self, fit_metrics_dict):
        ...
        self.append_metric_line_to_file(
            fit_metrics_dict,
            filename=self.ladder_metrics_filename,
    ...
    def cycle_step_callback(self, fit_metrics_dict):
        ...
        self.append_metric_line_to_file(
            fit_metrics_dict,
            filename=self.cycle_metrics_filename,
    ...
    def fit_metric_callback(self, fit_metrics_dict, extended_display_step=None):
        ...
        self.append_metric_line_to_file(
            fit_metrics_dict, filename=self.running_metrics_filename
        )
    ...
    def test_ladder_step_callback(self, fit_metrics_dict):
        ...
        self.append_metric_line_to_file(
            fit_metrics_dict,
            filename=self.test_ladder_metrics_filename,
            columns=self.ladder_metrics_columns,
        )

    def test_cycle_step_callback(self, fit_metrics_dict):
        self.print_extended_metrics(
            fit_metrics_dict, title="TEST Cycle last iteration:"
        )
        self.append_metric_line_to_file(
            fit_metrics_dict,
            filename=self.test_cycle_metrics_filename,
            columns=self.cycle_metrics_columns,
        )
    ...
    def test_metric_callback(self, metrics_dict, extended_display_step=None):
        ...
        self.append_metric_line_to_file(
            metrics_dict, filename=self.test_running_metrics_filename
        )
    ...
    def append_metric_line_to_file(
        self, fit_metrics_dict, filename="metrics.log", columns=None
    ):
        if columns is None:
            columns = MetricsAggregator.columns
        ...
        reg_loss = (
            fit_metrics_dict["l1_reg_contrib"] + fit_metrics_dict["l2_reg_contrib"]
        )
        ...
        fmt_string = " ".join(fmt_string_list)
        log_message = fmt_string.format(
            *[fit_metrics_dict.get(col, 0) for col in columns]
        )
    ...
    def print_extended_metrics(fit_metrics_dict, title="FIT_STATS"):
        ...
        str0 = "\n" + "-" * 44 + title + "-" * 44 + "\n"
        str1 = "{prefix:<11} #{iter_num:<4}".format(
            prefix="Iteration:", iter_num=iter_num
        )
        str1 += "{prefix:<8}".format(prefix="Loss:")
        str1 += "{prefix:>8} {tot_loss:>1.4e} ({fr:3.0f}%) ".format(
            prefix="Total: ", tot_loss=total_loss, fr=100
        )
        str1 += "\n"

        fr = (
            fit_metrics_dict["e_loss_contrib"] / total_loss * 100
            if total_loss > 0
            else 0
        )
        str2 = "{prefix:>33} {e_loss:>1.4e} ({fr:3.0f}%) ".format(
            prefix="Energy: ", e_loss=fit_metrics_dict["e_loss_contrib"], fr=fr
        )
        str2 += "\n"

        fr = (
            fit_metrics_dict["f_loss_contrib"] / total_loss * 100
            if total_loss > 0
            else 0
        )
        str3 = "{prefix:>33} {f_loss:>1.4e} ({fr:3.0f}%) ".format(
            prefix="Force: ", f_loss=fit_metrics_dict["f_loss_contrib"], fr=fr
        )
        str3 += "\n"
        ...
        str4 = "{prefix:>33} {l1:>1.4e} ({fr:3.0f}%) ".format(
            prefix="L1: ", l1=l1, fr=fr
        )
        str4 += "\n"
        fr = l2 / total_loss * 100 if total_loss != 0 else 0
        str4 += "{prefix:>33} {l2:>1.4e} ({fr:3.0f}%) ".format(
            prefix="L2: ", l2=l2, fr=fr
        )
        str4 += "\n"
        ...
        er_rmse += (
            "{:>14.2f}".format(fit_metrics_dict["rmse_epa"] * 1e3)
        ...
        er_mae += (
            "{:>14.2f}".format(fit_metrics_dict["mae_epa"] * 1e3)
        ...
        er_max += (
            "{:>14.2f}".format(fit_metrics_dict["max_abs_epa"] * 1e3)
    ...
    def print_detailed_metrics(fit_metrics_dict, title="Iteration:"):
        ...
        log.info(
            ...
            + "{:<14}".format(
                "({numeval} evals):".format(numeval=fit_metrics_dict["eval_count"])
            )
            + "{:>10}".format("Loss: ")
            + "{loss: >1.4e}".format(loss=total_loss)
            + "{str1:>21}{rmse_epa:>.2f} ({low_rmse_e:>.2f}) meV/at".format(
                str1=" | RMSE Energy(low): ",
                rmse_epa=1e3 * fit_metrics_dict["rmse_epa"],
                low_rmse_e=1e3 * fit_metrics_dict["low_rmse_epa"],
            )
            + "{str3:>16}{rmse_f_comp:>.2f} ({low_rmse_f_comp:>.2f}) meV/A".format(
                str3=" | Forces(low): ",
