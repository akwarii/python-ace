...
class FitBackendAdapter:
    ...
    def fit(
        ...
        trainable_parameters = fit_config.get(
                ...
                try:
                    ...
                    if self.backend_config.get(BACKEND_BATCH_SIZE_REDUCTION_KW, True):
                        ...
                        batch_size_reduction_factor = self.backend_config.get(
                            BACKEND_BATCH_SIZE_REDUCTION_FACTOR_KW, 1.618
                        )  # default - golden ratio
                        new_batch_size = int(batch_size / batch_size_reduction_factor)
                        log.info(
                            "Decrease batch size (by factor {}): {} -> {}".format(
                                batch_size_reduction_factor, batch_size, new_batch_size
                            )
                        )
                        if new_batch_size < 1:
                            log.error("New batch size is too small, stopping")
                            raise RuntimeError(
                                "No further batch size reduction is possible, stopping"
                            )
    ...
    def get_evaluator_version_dict(self) -> dict[str, str]:
        try:
            if self.backend_config.evaluator_name == TENSORPOT_EVAL:
                ...
                return {
                    PYACE_EVAL + "_version": __version__,
    ...
    def setup_tensorpot(
        ...
        with warnings.catch_warnings():
            ...
            has_smoothness = (
                np.array([loss_spec.w0_rad, loss_spec.w1_rad, loss_spec.w2_rad]) != 0
            ).any()
            ...
            if has_smoothness:
                aux_loss_factor += [
                    np.float64(loss_spec.w0_rad) / n_batches,
            ...
            ace_potential = ACE(
                bbasisconfig,
                compute_orthogonality=has_orthogonality,
    ...
    def run_tensorpot_fit(
        ...
        with warnings.catch_warnings():
            ...
            def adapted_callback(coeffs):
                new_config = self.fitter.tensorpot.potential.get_updated_config(
                    updating_coefs=coeffs
                )
    ...
    def setup_pyace(
        ...
        self.fitter = PyACEFit(
            basis=bbasisconfig,
            loss_spec=loss_spec,
            executors_kw_args=dict(
    ...
    def run_pyace_fit(
        ...
        self.fitter.fit(
            structures_dataframe=dataframe,
            method=fit_config[FIT_OPTIMIZER_KW],
            options=options,
