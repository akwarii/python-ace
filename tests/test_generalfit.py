...
all_coeffs_ref_L1_L2 = np.array(
    [
        1.0018251007755958,
        -4.1253112073243596e-05,
        0.00014407142339193204,
        0.0009068770358774021,
        0.9992726100949606,
        -4.13259661999816e-05,
        -0.00017630705959434005,
        -0.0003830962785025632,
        -0.0005154906879633648,
        0.9999695212061385,
        -0.00013976779272491084,
        -0.0003195121591480847,
        -0.00011907907090764517,
        0.9999978653303362,
        -1.7643155458789115e-05,
        -5.408171499482168e-05,
        -0.00136185568345254,
        -8.202601657756907e-05,
        0.9996260764520081,
        -0.0008506335381837143,
        -0.0003682686247714983,
        -4.007865025889646e-06,
        0.9999512806806697,
        -0.0001643737892630971,
        -0.007446305527441,
        -0.006626531801662597,
        0.0003178422095423563,
        -9.480242468406922e-07,
        -0.0002787050358002696,
        -0.0009355121130896069,
        -0.0034691168309473113,
        -0.003445195988518582,
        -0.0274420217071527,
        -0.03143593665206571,
        -0.02168378024001206,
        -0.0024502245681668603,
        0.010731828287725245,
        0.0045285190124818185,
        -0.007815100758348204,
        -0.0037161436343309375,
        0.021330957119601306,
        0.006383302099100587,
        -0.021505727564229957,
        -0.009390823019485213,
        0.003355190617433722,
        0.0017035344491587064,
        -0.0016477972002598286,
        -0.0008746819281863965,
        0.009177192742107034,
        0.004581127854813982,
        -0.004855538914570914,
        -0.0025661430034491554,
        0.02402951109823248,
        0.011559903509051287,
        -0.01403089611519704,
        -0.00722872163379242,
    ]
)
...
def compare_coefficients(
    all_coeffs, all_coeffs_ref, abs_threshold=2e-7, rel_threshold=2e-7
):
...
def test_GeneralACEFit_pyace_fit_options():
    ...
    fitace = GeneralACEFit(
        potential_config=fitcyles_potential_config,
    ...
    compare_coefficients(
        all_coeffs, [0, 0, 0, 0], abs_threshold=2e-5, rel_threshold=None
    )
...
def test_GeneralACEFit_pyace_fit_func_coefs_init_random():
    ...
    fitace = GeneralACEFit(
        potential_config=func_coefs_random_potential_config,
        data_config=test_data_config,
        fit_config=fit_config,
        backend_config=backend_config,
        seed=42,
    ...
    compare_coefficients(
        all_coeffs,
        [
            4.967141530112327e-05,
            -1.3826430117118467e-05,
            6.476885381006925e-05,
...
def test_GeneralACEFit_pyace_core_rep():
    ...
    fitace2 = GeneralACEFit(
        potential_config="test_core_rep.yaml",
        data_config=test_data_config,
        fit_config=fit_config,
        backend_config=backend_config,
        seed=42,
...
def test_GeneralACEFit_pyace_L1_L2():
    ...
    fitace = GeneralACEFit(
        potential_config=test_potential_config,
        data_config=test_data_config,
    ...
    all_coeffs = np.array(bbasisconfig.get_all_coeffs())
    compare_coefficients(
        all_coeffs, all_coeffs_ref_L1_L2, abs_threshold=2e-6, rel_threshold=1e-2
    )
...
all_coeffs_rad_smoothness = np.array(
    [
        1.0019169656619304,
        -4.647047170258803e-05,
        0.00014034280495604616,
        0.0009583564389256882,
        0.9989287434336567,
        -6.200445621404138e-05,
        -0.00027220425985198396,
        -0.0005788038038901483,
        -0.0008259408296999081,
        0.9999268660800964,
        -0.0002535510431383997,
        -0.0004927811018673341,
        -0.00036058978283052634,
        0.999959387379982,
        -0.0001117104695747868,
        -0.00018144431737907173,
        -0.0018175904349284641,
        -0.00012417967577225576,
        0.9994617646940869,
        -0.0011353382325416962,
        -0.000656176579733758,
        -3.582605059275514e-05,
        0.9998359760995102,
        -0.0003347322829450886,
        -0.007928538500798294,
        -0.006841249595933891,
        0.00029786760697641916,
        -5.7460132367418244e-05,
        -0.00035100203428952917,
        -0.0010001480494823871,
        -0.003778707408784413,
        -0.0035848036562923526,
        -0.02930579902132576,
        -0.031227653857346792,
        -0.022062769863560497,
        -0.003182125438847701,
        0.011189831983510106,
        0.004847572373215616,
        -0.008099217345682697,
        -0.003894375421558507,
        0.022058726015781967,
        0.007292296393542844,
        -0.022242843640854285,
        -0.00992428116873616,
        0.0035894323335002257,
        0.0018610830689793426,
        -0.0017960799591859307,
        -0.0009839527216469414,
        0.009673756300408058,
        0.004895115958719443,
        -0.00514442708024614,
        -0.002753293747719123,
        0.02503918938840678,
        0.012191732176742989,
        -0.014695291955885448,
        -0.007635408163554351,
    ]
)
...
def test_GeneralACEFit_pyace_loss_rad():
    ...
    compare_coefficients(
        all_coeffs, all_coeffs_rad_smoothness, abs_threshold=2e-6, rel_threshold=1e-2
    )
...
def test_GeneralACEFit_tensorpot_fit_options():
    ...
    # assert int(bbasisconfig.metadata["_fit_cycles"]) == 2
    compare_coefficients(
        all_coeffs, [0.0, 0.0, 0.0, 0.0], abs_threshold=2e-5, rel_threshold=None
    )
...
def test_GeneralACEFit_tensorpot_L1_L2():
    ...
    all_coeffs = fitace.target_bbasisconfig.get_all_coeffs()
    compare_coefficients(
        all_coeffs, all_coeffs_ref_L1_L2, abs_threshold=0.005, rel_threshold=None
    )
...
def test_GeneralACEFit_tensorpot_loss_rad():
    ...
    all_coeffs = np.array(bbasisconfig.get_all_coeffs())
    compare_coefficients(
        all_coeffs, all_coeffs_rad_smoothness, abs_threshold=0.005, rel_threshold=None
    )


all_coeffs_bbasis_yaml = [
    1.0156768899086037,
    7.0045191654986985,
    0.18102546004421222,
    7.530623283167457,
...
def test_GeneralACEFit_pyace_BBasis_yaml():
    ...
    compare_coefficients(
        all_coeffs, all_coeffs_bbasis_yaml, abs_threshold=7e-6, rel_threshold=2e-5
    )
...
def test_GeneralACEFit_tensorpot_BBasis_yaml():
    if not TENSORPOTENTIAL_IMPORTED:
        pytest.fail()
    backend_config = {"evaluator": "tensorpot", "batch_size": 2}  # pyace, tensorpot
    bbasis = ACEBBasisSet("tests/Al-r1234l12_crad_dif_2.yaml")
    fitace = GeneralACEFit(
        potential_config=bbasis,
        fit_config=test_fit_config_L1_L2,
    ...
    all_coeffs = bbasisconfig.get_all_coeffs()
    compare_coefficients(
        all_coeffs, all_coeffs_bbasis_yaml, abs_threshold=2e-6, rel_threshold=6e-6
    )
...
def test_GeneralACEFit_pyace_weighting():
    ...
    all_coeffs = bbasisconfig.get_all_coeffs()
    compare_coefficients(
        all_coeffs, all_coeffs_ref_weighting, abs_threshold=3e-6, rel_threshold=6e-6
    )
...
def test_GeneralACEFit_tensorpot_weighting():
    ...
    all_coeffs = bbasisconfig.get_all_coeffs()
    compare_coefficients(
        all_coeffs, all_coeffs_ref_weighting, abs_threshold=2e-6, rel_threshold=5e-6
    )
...
def test_GeneralACEFit_pyace_ladderscheme():
    ...
    fitace = GeneralACEFit(
        potential_config=ladder_potential_config,
        data_config=test_data_config,
    ...
    compare_coefficients(
        ...
        [
            0.9998508552498461,
            -0.0038456640969905514,
            0.9997419540364952,
            0.005679640585532174,
            -0.6537803633506181,
            3.1399397329259173,
            -0.0009856261099147837,
            -0.01723476703936692,
        ],
        abs_threshold=6e-3,
        rel_threshold=None,
...
def test_GeneralACEFit_pyace_ladderscheme_initial_potential():
    ...
    fitace = GeneralACEFit(
        potential_config=ladder_potential_config_init,
        data_config=test_data_config,
        fit_config=ladder_fit_config,
        backend_config=backend_config,
    )
    ...
    compare_coefficients(
        ...
        [
            0.9998687311259148,
            -0.002074779584948978,
            0.9997998833532301,
            0.0035680103476770156,
            -0.8729670926048945,
            2.4480651973606204,
            -4.1657079213402396e-05,
            -0.012264438891335432,
        ],
        abs_threshold=6e-3,
        rel_threshold=None,
    )
