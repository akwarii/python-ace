...
def test_constructor_from_bBasisFunctionSpecification():
    spec = BBasisFunctionSpecification(
        ["Al", "Al", "Al"], ns=[1, 1], ls=[1, 1], coeffs=[1.0]
    )
    spec2 = BBasisFunctionSpecification(
        ["Al", "Al", "Al", "Al", "Al"],
        ns=[2, 3, 3, 3],
        ls=[1, 1, 2, 2],
        LS=[2, 2],
...
def test_sort_order():
    ...
    spec = BBasisFunctionSpecification(
        ["Al", "Al", "Al", "Al"], ns=[1, 1, 1], ls=[2, 1, 1], LS=[1], coeffs=[1.0]
    )
    func = ACEBBasisFunction(spec)
...
def test_constructor_from_bBasisFunctionSpecification_multispecies_exception():
    spec = BBasisFunctionSpecification(
        ["Al", "Al", "Ni"], ns=[1, 1], ls=[1, 1], coeffs=[1.0]
    )
...
def test_constructor_from_bBasisFunctionSpecification_multispecies():
    spec = BBasisFunctionSpecification(
        ["Ni", "Al", "Ni"], ns=[1, 1], ls=[1, 1], coeffs=[1.0]
    )
