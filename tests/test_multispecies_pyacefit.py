...
df_mod["species"] = df_mod["ase_atoms"].map(
    lambda at: tuple(sorted(set(at.get_chemical_symbols())))
)
...
def test_fit_multispecies():
    ...
    assert np.allclose(
        fit.best_params[:5],
        [1.00394737, 2.00789474, 1.00394737, 2.00789474, 1.00394737],
    )
...
def test_fit_2comp_comp0_data0():
    ...
    np.random.seed(42)
    bbasisconf_Al = create_multispecies_basis_config(
        potential_config, func_coefs_initializer="random"
    )
    df = df_Al
    ...
    method = "BFGS"
    fit.fit(
        structures_dataframe=df, method=method, options={"maxiter": 1, "disp": True}
    )
...
def test_fit_2comp_comp1_data1():
    ...
    np.random.seed(42)
    bbasisconf_AlNi = create_multispecies_basis_config(
        potential_config, func_coefs_initializer="random"
    )
    df = df_Ni
    # tp = TensorPotential(potential, loss_specs={
...
def test_fit_ace_with_l1_l2_reg():
    ...
    loss_spec = LossFunctionSpecification(
        kappa=0.5, L1_coeffs=0.1 * 5, L2_coeffs=0.05 * 5, w1_coeffs=1.0, w2_coeffs=1.0
    )
